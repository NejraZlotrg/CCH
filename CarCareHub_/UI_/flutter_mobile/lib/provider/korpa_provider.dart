...
Future<bool> updateKolicina(int? korpaId, int? proizvodId, int novaKolicina) async {
  final url = Uri.parse("http://10.0.2.2:7209/api/korpa/$korpaId/proizvod/$proizvodId?novaKolicina=$novaKolicina");
...
Future<bool> ocistiKorpu({int? klijentId, int? zaposlenikId, int? autoservisId}) async {
  ...
  if (klijentId != null && zaposlenikId == null && autoservisId == null) {
    url = Uri.parse("http://10.0.2.2:7209/api/korpa/ocistiKorpu?klijentId=$klijentId");
  } else if (zaposlenikId != null && klijentId == null && autoservisId == null) {
    url = Uri.parse("http://10.0.2.2:7209/api/korpa/ocistiKorpu?zaposlenikId=$zaposlenikId");
  } else if (autoservisId != null && klijentId == null && zaposlenikId == null) {
    url = Uri.parse("http://10.0.2.2:7209/api/korpa/ocistiKorpu?autoservisId=$autoservisId");
  } else {
