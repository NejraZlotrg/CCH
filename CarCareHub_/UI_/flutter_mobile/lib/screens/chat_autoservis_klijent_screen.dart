  ...
  try {
    ...
    setState(() {
      ...
      if (index != -1) {
        ...
        // Determine if message should be bold based on sender and user role
        final shouldBeBold = (userProvider.role == 'Autoservis' && message.poslanoOdKlijenta) || 
  ...
  try {
    ...
    if (response.isNotEmpty) {
      for (var chat in response) {
        ...
        int? autoservisId = chat.autoservisId;
        final chatKey = _getChatKey(klijentId, autoservisId);
        
        var messages = await chatProvider.getMessages(klijentId, autoservisId);
        if (messages.isNotEmpty) {
          final lastMessage = messages.last;
          chat.poruka = lastMessage.poruka;
          
          // Determine if message should be bold based on sender and user role
          final shouldBeBold = (userProvider.role == 'Autoservis' && lastMessage.poslanoOdKlijenta) || 
