...
class FirmaAutodijelovaProvider extends BaseProvider<FirmaAutodijelova> {
 ...
 Future<int?> getIdByUsernameAndPassword(String username, String password) async {
  try {
    final response = await http.post(
      Uri.parse('http://localhost:7209/api/firmaAutodijelova/get-id?username=$username&password=$password'), 
      headers: {
 ...
 Future<bool?> getVidljivoByUsernameAndPassword(String username, String password) async {
  try {
    final response = await http.post(
      Uri.parse('http://localhost:7209/api/firmaAutodijelova/get-vidljivo?username=$username&password=$password'),  
      headers: {
        "Content-Type": "application/json",  
      },
      body: jsonEncode({
  ...
  Future<FirmaAutodijelova> getSingleById(int id) async {
    String url = "http://localhost:7209/api/firmaAutodijelova/FirmaAutodijelovaGetByID/$id"; 
...
Future<bool> checkUsernameExists(String username) async {
  try {
    final response = await http.get(
      Uri.parse('http://localhost:7209/api/firmaAutodijelova/check-username/$username'),
      headers: {
        "Content-Type": "application/json",
      },
    );
