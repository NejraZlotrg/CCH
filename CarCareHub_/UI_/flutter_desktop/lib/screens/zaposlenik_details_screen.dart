...
class ZaposlenikDetailsScreen extends StatefulWidget {
  ...
  @override
  State<ZaposlenikDetailsScreen> createState() =>
      _ZaposlenikDetailsScreenState();
}
      ...
      child: Column(
        ...
        children: [
          ...
          FormBuilderDropdown<String>(
            ...
            enabled: isAdminOrOwnProfile &&
                (selectedFirmaId == '' || selectedFirmaId == null),
            onChanged: (value) {
              setState(() {
                selectedAutoservisId = value ?? '';
                if (value != '') selectedFirmaId = '';
              });
            },
          ),
          const SizedBox(height: 15),
          FormBuilderDropdown<String>(
            ...
            items: [
              const DropdownMenuItem(
                value: '',
                child: Text('Izaberi firmu autodijelova',
                    style: TextStyle(color: Colors.grey)),
              ),
            ...
            enabled: isAdminOrOwnProfile &&
                (selectedAutoservisId == '' || selectedAutoservisId == null),
            onChanged: (value) {
              setState(() {
                selectedFirmaId = value ?? '';
                if (value != '') selectedAutoservisId = '';
              });
            },
          ),
          Padding(
                    ...
                    onPressed: () async {
                      if (_formKey.currentState?.saveAndValidate() ?? false) {
                        ...
                        if (username != null &&
                          ...
                          final exists = await _zaposlenikProvider
                              .checkUsernameExists(username);
                          if (exists &&
                              (widget.zaposlenik == null ||
                                  widget.zaposlenik?.username?.toLowerCase() !=
                        ...
                        final autoservisId = _formKey
                            .currentState?.fields['autoservisId']?.value;
                        final firmaId = _formKey
                            .currentState?.fields['firmaAutodijelovaId']?.value;
                        ...
                        if ((autoservisId == null || autoservisId == '') &&
                          ...
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                  'Morate odabrati ili autoservis ili firmu autodijelova.'),
                        ...
                        if (request['datumRodjenja'] != null) {
                          request['datumRodjenja'] =
                              (request['datumRodjenja'] as DateTime)
                                  .toIso8601String();
                        }
                        ...
                        if (selectedAutoservisId != null &&
                            selectedAutoservisId!.isNotEmpty) {
                          request['autoservisId'] =
                              int.tryParse(selectedAutoservisId!);
                          request['firmaAutodijelovaId'] = null;
                        } else if (selectedFirmaId != null &&
                            selectedFirmaId!.isNotEmpty) {
                          request['firmaAutodijelovaId'] =
                              int.tryParse(selectedFirmaId!);
