...
class _IzvjestajNarudzbiScreenState extends State<IzvjestajNarudzbiScreen> {
  ...
  Future<void> _downloadKlijentiReport() async {
    ...
    try {
      ...
      await _savePdfLocally(pdf, 'klijenti_izvjestaj_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf');
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(replaceDiacritics('Greska: $e'))),
      );
...
pw.Widget _buildCurrentDataPdfTable() {
  return pw.Table.fromTextArray(
    ...
    data: [
      ...
      ...izvjestaji.map((e) => [
        replaceDiacritics(e.datumNarudzbe != null ? _formatDate(e.datumNarudzbe!) : 'N/A'),
...
Future<void> _downloadCurrentReport() async {
  ...
  Future<void> _downloadZaposleniciReport() async {
    ...
    try {
      ...
      await _savePdfLocally(pdf, 'zaposlenici_izvjestaj_${DateFormat('yyyyMMdd').format(DateTime.now())}.pdf');
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text(replaceDiacritics('Greska: $e'))),
      );
  ...
  // PDF generators
  Future<pw.Document> _generateAutoservisPdf(List<AutoservisIzvjestaj> reportData) async {
    final pdf = pw.Document();
    pdf.addPage(
      pw.MultiPage(
        pageFormat: PdfPageFormat.a4,
  ...
  Future<pw.Document> _generateKlijentiPdf(List<KlijentIzvjestaj> reportData) async {
    final pdf = pw.Document();
    pdf.addPage(
      pw.MultiPage(
        pageFormat: PdfPageFormat.a4,
  ...
  Future<pw.Document> _generateZaposleniciPdf(List<ZaposlenikIzvjestaj> reportData) async {
    final pdf = pw.Document();
    pdf.addPage(
      pw.MultiPage(
        pageFormat: PdfPageFormat.a4,
        build: (context) => [
          pw.Header(
            level: 0,
            child: pw.Text(
              replaceDiacritics('Izjestaj za zaposlenike (posljednjih 30 dana)'),
              style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold),
            ),
          ),
          pw.SizedBox(height: 20),
 ...
 Future<void> _loadData(Map<String, dynamic> filter) async {
  try {
    ...
    if (filter['klijentIme'] != null && filter['klijentIme'].toString().isNotEmpty) {
    ...
    if (filter['zaposlenikIme'] != null && filter['zaposlenikIme'].toString().isNotEmpty) {
    ...
    if (filter['autoservisNaziv'] != null && filter['autoservisNaziv'].toString().isNotEmpty) {
      final searchTerm = filter['autoservisNaziv'].toString().toLowerCase();
      filteredData = filteredData.where((e) => 
        e.autoservis?.naziv?.toLowerCase().contains(searchTerm) ?? false
      ).toList();
    }
...
Widget build(BuildContext context) {
  return MasterScreenWidget(
    ...
    child: Container(
      color: const Color.fromARGB(255, 204, 204, 204), // Grey background for the entire screen
          ...
          children: [
            ...
            Padding(
              ...
              child: Column(
                children: [
                  ElevatedButton.icon(
  onPressed: _isLoadingCurrentReport ? null : _downloadCurrentReport,
  ...
  style: ElevatedButton.styleFrom(
    ...
    minimumSize: const Size(double.infinity, 48),
                  ...
                  const SizedBox(height: 10),
                  ElevatedButton.icon(
                    onPressed: _isLoadingKlijentiReport ? null : _downloadKlijentiReport,
                    icon: _isLoadingKlijentiReport 
                        ...
                        : const Icon(Icons.download),
                    label: const Text("Download izvještaj za sve klijente (30 dana)"),
                    style: ElevatedButton.styleFrom(
                  ...
                  const SizedBox(height: 10),
                  ElevatedButton.icon(
                    onPressed: _isLoadingZaposleniciReport ? null : _downloadZaposleniciReport,
                    icon: _isLoadingZaposleniciReport 
                        ...
                        : const Icon(Icons.download),
                    label: const Text("Download izvještaj za sve zaposlenike (30 dana)"),
                    style: ElevatedButton.styleFrom(
            ...
            const SizedBox(height: 10),
            // Wrap the DataTable in an Expanded or SizedBox with height
            Container(
              color: Colors.white, // White background for the table area
          ...
          child: Column(
            children: [
                ...
                children: [
                  Expanded(
                    child: FormBuilderTextField(
                      ...
                      onChanged: (value) => _onFieldChanged('klijentIme', value),
                    ),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: FormBuilderTextField(
                      ...
                      onChanged: (value) => _onFieldChanged('zaposlenikIme', value),
                    ),
                  ),
                ],
              ),
              ...
              FormBuilderTextField(
                ...
                onChanged: (value) => _onFieldChanged('autoservisNaziv', value),
              ),
