...
class _NarudzbaStavkaScreenState extends State<NarudzbaStavkaScreen> {
    ...
    return Column(
      ...
      children: [
        ...
        _buildDetailRow(
            "Datum narudžbe:",
            dateFormat
                .format(firstItem.narudzba?.datumNarudzbe ?? DateTime.now())),
      ...
      return FutureBuilder(
        ...
        builder: (context, snapshot) {
          ...
          return Column(
            ...
            children: [
              _buildDetailRow(
                  "Autoservis:", autoservis.naziv ?? "AC Herc - autoservis"),
          ...
          children: [
            TableRow(
              ...
              children: [
                ...
                const Padding(
                  padding: EdgeInsets.all(8),
                  child: Text("Osnovna (jedinična cijena)",
                      style: TextStyle(fontWeight: FontWeight.bold)),
                ),
                const Padding(
                  padding: EdgeInsets.all(8),
                  child: Text("Popust (jedinična cijena)",
                      style: TextStyle(fontWeight: FontWeight.bold)),
                ),
                if (isAutoservisUser)
                  const Padding(
                    padding: EdgeInsets.all(8),
                    child: Text("Cijena za autoservise (jedinična cijena)",
                        style: TextStyle(fontWeight: FontWeight.bold)),
                  ),
                const Padding(
                  padding: EdgeInsets.all(8),
                  child: Text("Fakturisano po jedinicnoj cijeni",
                      style: TextStyle(fontWeight: FontWeight.bold)),
                ),
                const Padding(
                  padding: EdgeInsets.all(8),
                  child: Text("Ukupno fakturisano po cijeni",
                      style: TextStyle(fontWeight: FontWeight.bold)),
                ),
              ],
            ),
            ...result!.result.map((stavka) {
              ...
              final autoservicePrice =
                  stavka.proizvod?.cijenaSaPopustomZaAutoservis;
              final firmaId = stavka.proizvod?.firmaAutodijelovaID;

              final hasRegularDiscount =
                  discountPrice != null && discountPrice < basePrice;
              final hasAutoserviceDiscount =
                  autoservicePrice != null && autoservicePrice < basePrice;
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: FutureBuilder<List<int>>(
                      future: firmaId != null
                          ? _getAutoservisIdsForFirma(firmaId)
                          : Future.value([]),
                      builder: (context, snapshot) {
                        ...
                        final narudzbaAutoservisId =
                            stavka.narudzba?.autoservisId;
                  ...
                  Padding(
                    ...
                    child: FutureBuilder<List<int>>(
                      ...
                      builder: (context, snapshot) {
                        ...
                        return Text(
                          shownPrice != null
                              ? _formatCurrency(shownPrice * kolicina)
                              : "N/A",
                          style: TextStyle(
                            color: isLinked ? Colors.black : null,
                          ),
  ...
  Widget _buildOrderSummary() {
    ...
    return Column(
      ...
      children: [
        _buildSummaryRow(
            "Ukupna cijena:", "${ukupnaCijena.toStringAsFixed(2)} KM"),
  ...
  Widget _buildStatusIndicator() {
    final isCompleted =
        result!.result.first.narudzba?.zavrsenaNarudzba ?? false;
